...
def main():
    # パラメータ設定
    N = 100
    sigma_max = 2.0  # 最大ノイズレベル
    sigma_values = np.arange(0.1, sigma_max + 0.1, 0.1)
    num_trials = 1000
    
    u_true = np.array([1/300**2, 0, 1/200**2, 0, 0, -1]) # 真の楕円パラメータベクトル
    ...
    plt.ylim(0, max(np.max(rms_least_squares), np.max(rms_maximum_likelihood)) * 1.1)
    
    plt.gca().yaxis.set_major_formatter(ticker.ScalarFormatter(useMathText=True))
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))

    # 図を保存
    plt.savefig('ellipse_fitting_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
