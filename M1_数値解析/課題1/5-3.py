...
def calculate_rms_error(u_estimates, u_true):
    ...
    for u in u_estimates:
        ...
        if np.dot(u, u_true) < 0:
            u = -u
        cos_angle = np.clip(np.dot(u, u_true), -1.0, 1.0)
        angle = np.arccos(cos_angle)
        error_angles.append(angle)
    return np.sqrt(np.mean(np.square(error_angles)))
...
def main():
    ...
    # ノイズ付加例の描画（sigma=3.0）
    x_noisy_3, y_noisy_3 = add_noise(x_true, y_true, 3.0)

    plt.figure(figsize=(8, 6))
    plt.plot(x_true, y_true, 'k-', label='True Ellipse')
    plt.scatter(x_noisy_3, y_noisy_3, s=5, c='red', alpha=0.5, label='Noisy Points ($\sigma=3.0$)')
    plt.axis('equal')
    plt.title('Ellipse Points with Noise')
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    for idx, sigma in enumerate(sigma_values):
        ...
        rms_ls_val = calculate_rms_error(u_ls_list, u_true)
        rms_mle_val = calculate_rms_error(u_mle_list, u_true)
        kcr_bound = calculate_kcr_lower_bound(x_true, y_true, u_true, sigma)

        rms_ls.append(rms_ls_val)
        rms_mle.append(rms_mle_val)
        kcr_bounds.append(kcr_bound)

        print(f"  LSM RMS = {rms_ls_val:.4e}, MLE RMS = {rms_mle_val:.4e}, KCR = {kcr_bound:.4e}")
