...
実行方法：
python Yu-gauss.py
- 入力画像は 'in_shrimp.png' と 'out_shrimp.png' を使用
- 出力は 'convergence.png' に収束過程のグラフを保存
- 推定された回転角度とスケールをコンソールに表示
"""
...
for y in range(int(1 + 4 * sigma)):
    ...
    for x in range(int(1 + 4 * sigma)):
        ...
        gaussian_x[y].append(
            -coef
            * (-offset_x / sigma**2)
            * np.exp(-1 * (offset_x**2 + offset_y**2) / (2 * sigma**2))
        )
        gaussian_y[y].append(
            -coef
            * (-offset_y / sigma**2)
...
# 画像の読み込み
img_in = cv2.imread("Fuji.jpg", cv2.IMREAD_GRAYSCALE)
img_out = cv2.imread("out-Fuji.png", cv2.IMREAD_GRAYSCALE)
# 画像の大きさ
...
for i in range(loop):
    ...
    print(i, "θ：", np.rad2deg(theta), "s：", s)
    all_s.append(s)
